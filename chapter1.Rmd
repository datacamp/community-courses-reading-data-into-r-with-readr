...
This occurs every time any `readr` function is called to import data. In fact, all `readr` functions that read in data has the arguments `col_names` and `col_type`, which allow column specifications. These arguments are useful when you know you want to import certain columns of the data with certain names or as certain types. The `readr` package does a great job guessing what each column type should be, but it's important to know that you can also customize this further. 
...
```{r}
library(readr)
download.file("http://s3.amazonaws.com/assets.datacamp.com/production/course_1561/datasets/Salaries.csv", "Salaries.csv")
```
