...
*** =instructions
- Use `write_rds()` to save the `trees1` data frame as `trees.rds`.
- Use `read_rds()` to restore `trees.rds` to an R object called `trees2`.
- Use `identical()` to determine whether `trees1` is the same as `trees2`.
...
```{r}
library(readr)
trees1 <- read_csv("http://s3.amazonaws.com/assets.datacamp.com/production/course_1561/datasets/trees.csv")
```

*** =sample_code
```{r}
# Save trees as trees.rds
write_rds(trees1, "trees.rds")

# Import trees.rds: trees2
trees2 <- read_rds("trees.rds")

# Check whether trees and trees2 are the same
identical(trees1, trees2)
```
...
test_output_contains("identical(trees1, trees2)")
test_error()
success_msg("Great work!")
```
