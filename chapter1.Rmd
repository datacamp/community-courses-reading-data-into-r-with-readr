...
Sometimes you will run into files containing columns of data that all line up on one side, but are "ragged" on the other side (like how R prints data frames to the console).  These so-called fixed width files can be read by `readr`'s `read_table()` function.
...
test_correct({test_object("names")}, 
             {
                 test_function("read_table", args = c("file", "col_names"))
                 test_error()
             })
